...
-- Create each target × phase work order with the target-specific schedule sent by
-- the work-plan builder. save_work_order_v2 synchronizes these dates to shifts,
-- so the same source of truth is visible in work orders, resource calendars and
-- employee shift lists.
...
begin
  ...
  loop
    ...
    target_start_date := nullif(target_value->>'start_date', '')::date;
    target_end_date := nullif(target_value->>'end_date', '')::date;
    ...
    if (target_start_date is null) <> (target_end_date is null) then
      raise exception 'Työkohteelle "%" tarvitaan sekä aloitus- että valmistumispäivä.', target_title using errcode = '23514';
    end if;
    if target_start_date is not null and target_end_date < target_start_date then
      raise exception 'Työkohteen "%" valmistumispäivä ei voi olla ennen aloitusta.', target_title using errcode = '23514';
    end if;
    if target_value ? 'phase_schedules'
       and jsonb_typeof(target_value->'phase_schedules') <> 'array' then
      raise exception 'Työkohteen "%" vaihekohtainen aikataulu on virheellinen.', target_title using errcode = '22023';
    end if;
    if jsonb_typeof(target_value->'phase_schedules') = 'array'
       and jsonb_array_length(target_value->'phase_schedules') <> phase_count then
      raise exception 'Työkohteen "%" vaihekohtainen aikataulu ei vastaa työvaiheiden määrää.', target_title using errcode = '23514';
    end if;
    ...
    loop
      ...
      phase_schedule := case
        when jsonb_typeof(target_value->'phase_schedules') = 'array'
          then (target_value->'phase_schedules')->(phase_index - 1)
        else null
      end;
      effective_start_date := coalesce(
        nullif(phase_schedule->>'start_date', '')::date,
        (phase_value->>'start_date')::date
      );
      effective_end_date := coalesce(
        nullif(phase_schedule->>'end_date', '')::date,
        (phase_value->>'end_date')::date
      );

      if effective_start_date is null or effective_end_date is null then
        raise exception 'Kohteen "%" / työvaiheen "%" aikataulu puuttuu.',
          target_title, btrim(phase_value->>'title') using errcode = '23514';
      end if;
      if effective_end_date < effective_start_date then
        raise exception 'Kohteen "%" / työvaiheen "%" valmistuminen ei voi olla ennen aloitusta.',
          target_title, btrim(phase_value->>'title') using errcode = '23514';
      end if;
      if target_start_date is not null
         and (effective_start_date < target_start_date or effective_end_date > target_end_date) then
        raise exception 'Kohteen "%" työvaiheen "%" aikataulu ylittää kohteen aikavälin.',
          target_title, btrim(phase_value->>'title') using errcode = '23514';
      end if;
  ...
  -- Make the high-level phase timeline cover the actual target-specific work
  -- order dates. This keeps the Aikataulutus view consistent with calendars.
  update public.project_phases phase
  set start_date = schedule.minimum_start_date,
      end_date = schedule.maximum_end_date,
      updated_at = statement_timestamp()
  from (
    select
      work_order.project_phase_id,
      min(work_order.planned_start_date) as minimum_start_date,
      max(work_order.planned_end_date) as maximum_end_date
    from public.work_orders work_order
    where work_order.work_plan_id = v_work_plan_id
      and work_order.project_phase_id is not null
      and work_order.planned_start_date is not null
      and work_order.planned_end_date is not null
    group by work_order.project_phase_id
  ) schedule
  where phase.id = schedule.project_phase_id
    and phase.work_plan_id = v_work_plan_id;
  ...
  insert into public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'target_schedules_enabled', true,
      'calendar_sync_enabled', default_calendar_sync
    )
  );
