...
-- Work-order assignees must first be members of the project team.
...
begin
  ...
  if exists (
    ...
    where not exists (
      ...
      where om.organization_id = p_organization_id and om.user_id = requested.user_id
    )
  ) then
    raise exception 'Projektitiimiin voi lisätä vain organisaation käyttäjiä.' using errcode = '23503';
  end if;

  -- A removed installer must not retain a responsibility link to this project.
  ...
  insert into public.audit_logs (
    ...
    p_organization_id, auth.uid(), 'project_members_replaced', 'project_members',
    p_project_id, jsonb_build_object('member_user_ids', normalized_user_ids)
  );
end;
$$;
