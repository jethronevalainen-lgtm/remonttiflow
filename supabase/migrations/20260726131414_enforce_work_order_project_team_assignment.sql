...
-- Project work orders are assigned only to people who have first been placed
-- on that project's team. This keeps a worker's project access, work orders
-- and responsibility list in sync.
...
begin
  ...
  if exists (
    select 1 from unnest(normalized_user_ids) requested(user_id)
    where not exists (
  ...
  -- Removing a person from the project also removes their active responsibility
  -- links, so they cannot keep a work order from a project they no longer join.
...
begin
  ...
  if exists (
    ...
    raise exception 'Työmääräyksen vastuuhenkilön täytyy kuulua projektitiimiin.' using errcode = '23503';
  end if;
  ...
  where work_order_id = result_id and organization_id = p_organization_id;
  ...
  insert into public.audit_logs (
    ...
    'work_orders', result_id,
    jsonb_build_object('assignment_scope', p_assignment_scope, 'assignee_user_ids', normalized_user_ids, 'status', p_status)
  );
