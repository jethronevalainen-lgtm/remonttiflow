                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      <div className="mt-3 max-h-56 space-y-2 overflow-y-auto pr-1">
                        {availablePeople.map((person) => {
                          ...
                          return <label key={person.userId} className={cn('flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition', selected ? 'border-primary/35 bg-white' : 'border-transparent hover:bg-white')}><Checkbox checked={selected} onCheckedChange={(checked) => toggleAssignee(phase.id, person.userId, checked === true)} /><span className="min-w-0"><span className="block truncate text-sm font-medium">{person.name}</span><span className="block text-xs text-text-secondary">{person.role === 'worker' ? 'Työntekijä' : person.role === 'supervisor' ? 'Työnjohtaja' : person.role === 'project_coordinator' ? 'Projektikoordinaattori' : 'Ylläpitäjä'}</span></span></label>;
                        })}
