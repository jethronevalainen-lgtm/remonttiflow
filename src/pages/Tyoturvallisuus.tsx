...
function typeIcon(type: SafetyItemType) {
  ...
  if (type === 'inspection') return ClipboardCheck;
...
export default function Tyoturvallisuus() {
  ...
  const overdueCount = openItems.filter((item) => item.dueDate && item.dueDate < today).length;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-hero text-text-primary">Työturvallisuus</h1><p className="mt-1 text-body-sm text-text-secondary">Havainnosta vastuuhenkilölle, korjaukseen ja varmennettuun sulkemiseen</p></div><Button onClick={openCreate}><Plus size={16} className="mr-2" /> Uusi havainto</Button></div>
      ...
      <div className="space-y-3">
        {filteredItems.map((item) => {
          ...
          const overdue = item.dueDate && item.dueDate < today && !['Vahvistettu', 'Suljettu'].includes(item.status);
          return <Card key={item.id} className={overdue ? 'border-red-200' : ''}><CardContent className="p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="flex min-w-0 gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary-light"><Icon size={18} className="text-primary" /></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold">{item.title}</h2>{severityBadge(item.severity)}{statusBadge(item.status)}{overdue && <Badge className="border-0 bg-red-600 text-white">Myöhässä</Badge>}</div><p className="mt-1 text-xs font-medium text-text-muted">{typeLabel(item.type)}</p><p className="mt-2 whitespace-pre-wrap text-sm text-text-secondary">{item.description || 'Ei tarkempaa kuvausta.'}</p><div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs text-text-muted"><span>{item.date}</span><span>{item.project || 'Ei projektia'}</span>{item.location && <span className="flex items-center gap-1"><MapPin size={12} />{item.location}</span>}{item.assignee && <span className="flex items-center gap-1"><UserRound size={12} />{item.assignee}</span>}{item.dueDate && <span>Määräaika {item.dueDate}</span>}</div>{item.correctiveAction && <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm"><strong>Korjaava toimi:</strong> {item.correctiveAction}</div>}{item.rootCause && <p className="mt-2 text-xs text-text-secondary"><strong>Juurisyy:</strong> {item.rootCause}</p>}</div></div><div className="flex shrink-0 flex-wrap justify-end gap-1">{canManage && item.status === 'Ilmoitettu korjatuksi' && <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" onClick={() => void quickStatus(item, 'Vahvistettu')}><CheckCircle2 size={14} className="mr-1" /> Vahvista</Button>}{canManage && <><Button variant="outline" size="sm" onClick={() => openEdit(item)}><Edit3 size={14} className="mr-1" /> Muokkaa</Button><Button variant="ghost" size="sm" className="text-red-600" onClick={() => setDeleteTarget(item)}><Trash2 size={15} /></Button></>}</div></div></CardContent></Card>;
        })}
        {!filteredItems.length && <Card><CardContent className="p-12 text-center"><ShieldCheck size={44} className="mx-auto mb-3 text-text-muted" /><p className="font-semibold">Ei turvallisuustietoja</p></CardContent></Card>}
      </div>

      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
          <DialogHeader><DialogTitle>{editing ? 'Muokkaa turvallisuusasiaa' : 'Uusi turvallisuushavainto'}</DialogTitle></DialogHeader>
          {formErrors.length > 0 && <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{formErrors.map((error) => <p key={error}>{error}</p>)}</div>}
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2"><Label>Tyyppi</Label><Select value={form.type} onValueChange={(type: SafetyItemType) => setForm((previous) => ({ ...previous, type }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SAFETY_TYPES.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select></div>
            <div className="space-y-2"><Label>Päivä *</Label><Input type="date" value={form.date} onChange={(event) => setForm((previous) => ({ ...previous, date: event.target.value }))} /></div>
            <div className="space-y-2 sm:col-span-2"><Label>Otsikko *</Label><Input value={form.title} onChange={(event) => setForm((previous) => ({ ...previous, title: event.target.value }))} /></div>
            <div className="space-y-2 sm:col-span-2"><Label>Kuvaus *</Label><Textarea value={form.description} onChange={(event) => setForm((previous) => ({ ...previous, description: event.target.value }))} rows={4} /></div>
            ...
            <div className="space-y-2"><Label>Tarkka sijainti</Label><Input value={form.location} onChange={(event) => setForm((previous) => ({ ...previous, location: event.target.value }))} /></div>
            <div className="space-y-2"><Label>Vakavuus</Label><Select value={form.severity} onValueChange={(severity: SafetyForm['severity']) => setForm((previous) => ({ ...previous, severity }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Ei määritelty">Ei määritelty</SelectItem>{SEVERITIES.map((severity) => <SelectItem key={severity} value={severity}>{severity}</SelectItem>)}</SelectContent></Select></div>
            <div className="space-y-2"><Label>Tila</Label><Select value={form.status} onValueChange={(status) => setForm((previous) => ({ ...previous, status }))} disabled={!canManage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{STATUSES.map((status) => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select></div>
            ...
            <div className="space-y-2"><Label>Korjauksen määräaika</Label><Input type="date" value={form.dueDate} onChange={(event) => setForm((previous) => ({ ...previous, dueDate: event.target.value }))} /></div>
            <div className="space-y-2 sm:col-span-2"><Label>Juurisyy</Label><Textarea value={form.rootCause} onChange={(event) => setForm((previous) => ({ ...previous, rootCause: event.target.value }))} rows={2} /></div>
            <div className="space-y-2 sm:col-span-2"><Label>Korjaava toimenpide</Label><Textarea value={form.correctiveAction} onChange={(event) => setForm((previous) => ({ ...previous, correctiveAction: event.target.value }))} rows={3} /></div>
            <div className="space-y-2 sm:col-span-2"><Label>Ehkäisevä toimenpide</Label><Textarea value={form.preventiveAction} onChange={(event) => setForm((previous) => ({ ...previous, preventiveAction: event.target.value }))} rows={3} /></div>
          </div>
