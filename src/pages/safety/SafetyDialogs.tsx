...
export function SafetyItemDialog(props: SafetyItemDialogProps) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[94vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{editing ? 'Käsittele turvallisuusasiaa' : SAFETY_ACTIONS.find((item) => item.value === form.type)?.label}</DialogTitle></DialogHeader>{errors.length > 0 && <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{errors.map((item) => <p key={item}>{item}</p>)}</div>}<div className="grid gap-4 sm:grid-cols-2">
    ...
    <div className="space-y-2"><Label>Projekti *</Label><Select value={form.projectId} onValueChange={(value) => onForm((previous) => ({ ...previous, projectId: value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{projects.map((project) => <SelectItem key={project.id} value={project.id}>{project.name}</SelectItem>)}</SelectContent></Select></div>
    <div className="space-y-2"><Label>Tarkka sijainti</Label><Input value={form.location} onChange={(event) => onForm((previous) => ({ ...previous, location: event.target.value }))} /></div>
    ...
    {canManage && <><div className="space-y-2"><Label>Vastuuhenkilö</Label><Select value={form.assigneeUserId || 'none'} onValueChange={(value) => onForm((previous) => ({ ...previous, assigneeUserId: value === 'none' ? '' : value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Ei vastuuhenkilöä</SelectItem>{people.map((person) => <SelectItem key={person.userId} value={person.userId}>{person.name}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Korjauksen määräaika</Label><Input type="date" value={form.dueDate} onChange={(event) => onForm((previous) => ({ ...previous, dueDate: event.target.value }))} /></div><div className="space-y-2 sm:col-span-2"><Label>Juurisyy</Label><Textarea rows={2} value={form.rootCause} onChange={(event) => onForm((previous) => ({ ...previous, rootCause: event.target.value }))} /></div><div className="space-y-2 sm:col-span-2"><Label>Korjaava toimenpide</Label><Textarea rows={3} value={form.correctiveAction} onChange={(event) => onForm((previous) => ({ ...previous, correctiveAction: event.target.value }))} /></div><div className="space-y-2 sm:col-span-2"><Label>Ehkäisevä toimenpide</Label><Textarea rows={3} value={form.preventiveAction} onChange={(event) => onForm((previous) => ({ ...previous, preventiveAction: event.target.value }))} /></div></>}
  ...
  </div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Peruuta</Button><Button onClick={onSave} disabled={saving}>{saving ? 'Tallennetaan…' : 'Tallenna'}</Button></DialogFooter></DialogContent></Dialog>;
}
...
export function SafetyBriefingDialog(props: SafetyBriefingDialogProps) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[94vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{form.id ? 'Muokkaa turvallisuusohjetta' : 'Uusi turvallisuusohje'}</DialogTitle></DialogHeader>{errors.length > 0 && <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{errors.map((item) => <p key={item}>{item}</p>)}</div>}<div className="grid gap-4 sm:grid-cols-2">
    <div className="space-y-2 sm:col-span-2"><Label>Otsikko *</Label><Input value={form.title} onChange={(event) => onForm((previous) => ({ ...previous, title: event.target.value }))} /></div>
  ...
  </div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Peruuta</Button><Button onClick={onSave} disabled={saving}>{form.status === 'published' ? 'Julkaise ohje' : 'Tallenna luonnos'}</Button></DialogFooter></DialogContent></Dialog>;
}
...
export function SafetyProfileDialog({ open, form, saving, onOpenChange, onForm, onSave }: { open: boolean; form: ProjectSafetyProfile; saving: boolean; onOpenChange: (open: boolean) => void; onForm: Dispatch<SetStateAction<ProjectSafetyProfile>>; onSave: () => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[94vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>Työmaan hätä- ja turvallisuustiedot</DialogTitle></DialogHeader><div className="grid gap-4 sm:grid-cols-2">
    <div className="space-y-2 sm:col-span-2"><Label>Osoite</Label><Input value={form.siteAddress} onChange={(event) => onForm((previous) => ({ ...previous, siteAddress: event.target.value }))} /></div><div className="space-y-2"><Label>Kokoontumispaikka</Label><Input value={form.assemblyPoint} onChange={(event) => onForm((previous) => ({ ...previous, assemblyPoint: event.target.value }))} /></div><div className="space-y-2"><Label>Ensiapuvälineet</Label><Input value={form.firstAidLocation} onChange={(event) => onForm((previous) => ({ ...previous, firstAidLocation: event.target.value }))} /></div><div className="space-y-2"><Label>Defibrillaattori</Label><Input value={form.defibrillatorLocation} onChange={(event) => onForm((previous) => ({ ...previous, defibrillatorLocation: event.target.value }))} /></div><div className="space-y-2"><Label>Päivystysnumero</Label><Input type="tel" value={form.dutyPhone} onChange={(event) => onForm((previous) => ({ ...previous, dutyPhone: event.target.value }))} /></div><div className="space-y-2"><Label>Turvallisuusvastaava</Label><Input value={form.safetyContactName} onChange={(event) => onForm((previous) => ({ ...previous, safetyContactName: event.target.value }))} /></div><div className="space-y-2"><Label>Puhelin</Label><Input type="tel" value={form.safetyContactPhone} onChange={(event) => onForm((previous) => ({ ...previous, safetyContactPhone: event.target.value }))} /></div><div className="space-y-2"><Label>Ensiapuvastaava</Label><Input value={form.firstAidContactName} onChange={(event) => onForm((previous) => ({ ...previous, firstAidContactName: event.target.value }))} /></div><div className="space-y-2"><Label>Puhelin</Label><Input type="tel" value={form.firstAidContactPhone} onChange={(event) => onForm((previous) => ({ ...previous, firstAidContactPhone: event.target.value }))} /></div><div className="space-y-2 sm:col-span-2"><Label>Muut hätäohjeet</Label><Textarea rows={5} value={form.emergencyInstructions} onChange={(event) => onForm((previous) => ({ ...previous, emergencyInstructions: event.target.value }))} /></div>
  </div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Peruuta</Button><Button onClick={onSave} disabled={saving}>Tallenna</Button></DialogFooter></DialogContent></Dialog>;
}
...
export function EmergencyGuideDialog({ open, profile, project, onOpenChange, onCall }: { open: boolean; profile?: ProjectSafetyProfile; project?: SafetyProjectOption; onOpenChange: (open: boolean) => void; onCall: () => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle className="flex items-center gap-2 text-red-800"><HeartPulse size={20} /> Toimi hätätilanteessa</DialogTitle></DialogHeader><ol className="space-y-3 text-sm leading-6"><li className="rounded-xl border border-red-200 bg-red-50 p-4"><strong>1. Soita 112.</strong> Kerro mitä tapahtui ja kuinka monta loukkaantunutta on.</li><li className="rounded-xl border p-4"><strong>2. Ilmoita osoite:</strong> {profile?.siteAddress || project?.location || 'ei määritelty'}</li><li className="rounded-xl border p-4"><strong>3. Järjestä opastus.</strong> Lähetä henkilö sisäänkäynnille.</li><li className="rounded-xl border p-4"><strong>4. Estä lisävahingot turvallisesti.</strong></li></ol>{profile?.emergencyInstructions && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm"><strong>Työmaan oma ohje:</strong><p className="mt-1 whitespace-pre-wrap">{profile.emergencyInstructions}</p></div>}<DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Sulje</Button><Button className="bg-red-600 hover:bg-red-700" onClick={onCall}><Phone size={15} className="mr-2" /> Soita 112</Button></DialogFooter></DialogContent></Dialog>;
}
...
export function SafetyConfirmDialogs({ callOpen, deleteTarget, archiveTarget, address, onCallOpen, onDeleteTarget, onArchiveTarget, onDelete, onArchive }: { callOpen: boolean; deleteTarget: SafetyItem | null; archiveTarget: SafetyBriefing | null; address: string; onCallOpen: (open: boolean) => void; onDeleteTarget: (item: SafetyItem | null) => void; onArchiveTarget: (item: SafetyBriefing | null) => void; onDelete: () => void; onArchive: () => void }) {
  return <><AlertDialog open={callOpen} onOpenChange={onCallOpen}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Oletko hätätilanteessa?</AlertDialogTitle><AlertDialogDescription>Soita 112 vain todellisessa hätätilanteessa. Ilmoita osoite: {address || 'ei määritelty'}.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Peruuta</AlertDialogCancel><AlertDialogAction asChild className="bg-red-600 hover:bg-red-700"><a href="tel:112"><Phone size={15} className="mr-2" /> Soita 112</a></AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
