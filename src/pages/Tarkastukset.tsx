...
export default function Tarkastukset() {
  ...
  if (!currentOrg) return <Card><CardContent className="p-6 text-sm text-text-secondary">Valitse organisaatio ennen tarkastusten käyttöä.</CardContent></Card>;

  if (selectedInspectionId) {
    return <InspectionDetailView inspectionId={selectedInspectionId} canManage={canManage} currentRole={currentRole} organizationId={currentOrg.id} userId={user?.id} projects={projects} units={workspace.units} people={workspace.people} onBack={() => setSelectedInspectionId(null)} onWorkspaceRefresh={workspace.refresh} />;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><h1 className="text-hero text-text-primary">Tarkastukset ja itselleluovutukset</h1><p className="mt-1 max-w-3xl text-body-sm text-text-secondary">Huoneistotarkastukset, remonttien itselleluovutukset, puutteiden korjausseuranta ja hyväksytyt raportit.</p></div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="overview" className="mt-5 space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {metricCards.map((item) => <button key={item.label} type="button" className="text-left" onClick={() => setTab(item.tab)}><Card className="h-full transition-shadow hover:shadow-md"><CardContent className="p-5"><div className="mb-4 flex items-center justify-between"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-light"><item.icon size={19} className="text-primary" /></div><ArrowRight size={16} className="text-text-muted" /></div><p className="font-mono text-3xl font-bold">{item.value}</p><p className="font-semibold">{item.label}</p><p className="mt-1 text-xs text-text-secondary">{item.detail}</p></CardContent></Card></button>)}
          </div>
          <div className="grid gap-6 xl:grid-cols-[1.3fr_1fr]">
            <Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle className="flex items-center gap-2 text-lg"><ClipboardCheck size={18} />Seuraavat tarkastukset</CardTitle><Button variant="ghost" size="sm" onClick={() => setTab('inspections')}>Kaikki <ArrowRight size={14} className="ml-1" /></Button></CardHeader><CardContent className="space-y-2">{upcomingInspections.slice(0, 6).map((inspection) => <button key={inspection.id} type="button" onClick={() => setSelectedInspectionId(inspection.id)} className="w-full rounded-xl border p-3 text-left hover:bg-slate-50"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate font-semibold">{inspection.title}</p><p className="mt-1 text-xs text-text-secondary">{projectName(projects, inspection.projectId)} · {unitLabel(workspace.units, inspection.unitId)}</p></div><div className="text-right"><Badge className={cn('border-0', inspectionStatusClasses(inspection.status))}>{inspection.status}</Badge><p className="mt-1 text-xs">{formatDate(inspection.scheduledDate)}</p></div></div><Progress value={inspection.progress} className="mt-3 h-1.5" /></button>)}{upcomingInspections.length === 0 && <p className="py-8 text-center text-sm text-text-secondary">Ei tarkastuksia seuraavan seitsemän päivän aikana.</p>}</CardContent></Card>
            <Card><CardHeader><CardTitle className="flex items-center gap-2 text-lg"><AlertTriangle size={18} />Kiireelliset puutteet</CardTitle></CardHeader><CardContent className="space-y-2">{[...criticalFindings, ...overdueFindings.filter((item) => item.severity !== 'Kriittinen')].slice(0, 7).map((finding) => <button key={finding.id} type="button" onClick={() => setSelectedInspectionId(finding.inspectionId)} className="flex w-full items-start gap-3 rounded-xl border p-3 text-left hover:bg-slate-50"><span className={cn('mt-1 h-2.5 w-2.5 shrink-0 rounded-full', finding.severity === 'Kriittinen' ? 'bg-red-500' : 'bg-amber-500')} /><div className="min-w-0"><p className="truncate font-semibold">{finding.title}</p><p className="mt-1 text-xs text-text-secondary">{projectName(projects, finding.projectId)} · {unitLabel(workspace.units, finding.unitId)} · {formatDate(finding.dueDate)}</p></div></button>)}{criticalFindings.length + overdueFindings.length === 0 && <p className="py-8 text-center text-sm text-text-secondary">Ei kiireellisiä tarkastuspuutteita.</p>}</CardContent></Card>
          </div>
        </TabsContent>

        <TabsContent value="inspections" className="mt-5 space-y-4">
          <div className="flex flex-col gap-2 sm:flex-row"><div className="relative flex-1"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="pl-9" placeholder="Hae tarkastuksia…" /></div><Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="w-full sm:w-56"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Kaikki tilat</SelectItem>{(['Luonnos','Suunniteltu','Käynnissä','Puutteita havaittu','Korjattavana','Uusintatarkastus','Hyväksyttävänä','Hyväksytty','Mitätöity'] as InspectionStatus[]).map((status) => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select><Button variant="outline" size="icon" onClick={() => void workspace.refresh()}><RefreshCw size={16} /></Button></div>
          <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-3">{filteredInspections.map((inspection) => <button key={inspection.id} type="button" onClick={() => setSelectedInspectionId(inspection.id)} className="text-left"><Card className="h-full hover:shadow-md"><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-light"><ClipboardList size={18} className="text-primary" /></div><Badge className={cn('border-0', inspectionStatusClasses(inspection.status))}>{inspection.status}</Badge></div><h3 className="mt-4 font-semibold">{inspection.title}</h3><p className="mt-1 text-sm text-text-secondary">{projectName(projects, inspection.projectId)} · {unitLabel(workspace.units, inspection.unitId)}</p><p className="mt-2 text-xs text-text-muted">{inspection.inspectionType} · {formatDate(inspection.scheduledDate)}</p><div className="mt-4 flex justify-between text-xs"><span>Eteneminen</span><strong>{inspection.progress}%</strong></div><Progress value={inspection.progress} className="mt-1.5" /></CardContent></Card></button>)}{filteredInspections.length === 0 && <Card className="lg:col-span-2 xl:col-span-3"><CardContent className="p-10 text-center text-sm text-text-secondary">Hakuehdoilla ei löytynyt tarkastuksia.</CardContent></Card>}</div>
        </TabsContent>
        ...
        <TabsContent value="templates" className="mt-5 space-y-4">
          {currentRole === 'admin' && <Button onClick={() => { setEditingTemplate(null); setTemplateOpen(true); }}><Plus size={16} className="mr-2" />Luo oma pohja</Button>}
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{workspace.templates.map((template) => <Card key={template.id}><CardContent className="p-5"><div className="flex items-start justify-between"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-light"><ListChecks size={18} className="text-primary" /></div><div className="flex gap-2"><Badge variant="outline">v{template.version}</Badge>{template.system && <Badge className="border-0 bg-blue-50 text-blue-700">VaKantti</Badge>}</div></div><h3 className="mt-4 font-semibold">{template.name}</h3><p className="mt-1 text-xs font-medium uppercase tracking-wide text-text-muted">{template.category}</p><p className="mt-2 min-h-10 text-sm text-text-secondary">{template.description}</p>{currentRole === 'admin' && !template.system && <Button variant="outline" size="sm" className="mt-4" onClick={() => { setEditingTemplate(template); setTemplateOpen(true); }}>Luo uusi versio</Button>}</CardContent></Card>)}</div>
        </TabsContent>

        <TabsContent value="reports" className="mt-5"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{workspace.reports.map((report) => { const inspection = workspace.inspections.find((item) => item.id === report.inspectionId); return <Card key={report.id}><CardContent className="p-5"><div className="flex items-start justify-between"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50"><FileText size={18} className="text-emerald-700" /></div><Badge variant="outline">Versio {report.version}</Badge></div><h3 className="mt-4 font-semibold">{inspection?.title ?? 'Hyväksytty tarkastus'}</h3><p className="mt-1 text-sm text-text-secondary">{inspection ? `${projectName(projects, inspection.projectId)} · ${unitLabel(workspace.units, inspection.unitId)}` : 'Arkistoitu raportti'}</p><p className="mt-2 text-xs text-text-muted">Muodostettu {formatDateTime(report.generatedAt)}</p><Button variant="outline" size="sm" className="mt-4" onClick={() => setSelectedInspectionId(report.inspectionId)}><Printer size={15} className="mr-2" />Avaa ja tulosta PDF</Button></CardContent></Card>; })}{workspace.reports.length === 0 && <Card className="sm:col-span-2 xl:col-span-3"><CardContent className="p-10 text-center text-sm text-text-secondary">Hyväksyttyjä raporttiversioita ei ole vielä.</CardContent></Card>}</div></TabsContent>
      </Tabs>
