                      ...
                      {workPackage.orders.map((order) => {
                        ...
                        return (
                          <div key={order.id} className="grid gap-3 p-4 sm:grid-cols-[38px_1fr_auto] sm:items-center">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => navigate(`/tyomaaraykset?edit=${encodeURIComponent(order.id)}`)} aria-label={`Avaa työvaihe ${order.title}`}><Pencil size={16} /></Button>
                          </div>
                        );
                      })}
                    </div>
