...
function shiftTone(type: string) {
  ...
  if (normalized.includes('sair')) return 'border-red-200 bg-red-50 text-red-800';
  if (normalized.includes('koul')) return 'border-blue-200 bg-blue-50 text-blue-800';
...
export default function Tyovuorokalenteri() {
  ...
  const employeeNames = useMemo(() => {
    const names = new Set([
      ...employees.filter((employee) => employee.status !== 'Eroonnut').map((employee) => employee.name),
      ...weekShifts.map((shift) => shift.employeeName).filter(Boolean),
    ]);
    return [...names].sort((a, b) => a.localeCompare(b, 'fi'));
  }, [employees, weekShifts]);

  const totalHours = weekShifts.reduce((sum, shift) => sum + calculateHours(shift.startTime, shift.endTime), 0);
  const uncoveredDays = days.filter((day) => !weekShifts.some((shift) => shift.date === isoDate(day))).length;
  ...
  const selectEmployee = (employeeId: string) => {
    ...
    setForm((previous) => ({ ...previous, employeeId, employeeName: employee?.name ?? previous.employeeName }));
  };
  ...
  const save = async () => {
    ...
    if (form.startTime && form.endTime && form.endTime <= form.startTime) nextErrors.push('Loppuajan pitää olla alkuajan jälkeen.');
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-body-sm text-text-secondary">Henkilöstön viikkosuunnittelu ja projektikohdistus</p>
        </div>
        <Button onClick={() => openCreate()} className="gap-2"><Plus size={16} /> Lisää vuoro</Button>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={() => setWeekStart((previous) => addDays(previous, -7))}><ChevronLeft size={16} /></Button><Button variant="outline" size="sm" onClick={() => setWeekStart(startOfWeek(new Date()))}>Tämä viikko</Button><Button variant="outline" size="sm" onClick={() => setWeekStart((previous) => addDays(previous, 7))}><ChevronRight size={16} /></Button></div>
        <p className="font-semibold text-text-primary">{weekStart.toLocaleDateString('fi-FI')} – {weekEnd.toLocaleDateString('fi-FI')}</p>
      </div>
      ...
      <Card className="overflow-x-auto border-slate-200 shadow-card">
        <CardContent className="min-w-[1100px] p-0">
          <div className="grid grid-cols-[180px_repeat(7,minmax(125px,1fr))] border-b bg-slate-50">
            ...
            {days.map((day) => <div key={day.toISOString()} className="border-r p-3 text-center"><p className="text-xs font-semibold uppercase text-text-muted">{day.toLocaleDateString('fi-FI', { weekday: 'short' })}</p><p className="font-semibold text-text-primary">{day.toLocaleDateString('fi-FI', { day: 'numeric', month: 'numeric' })}</p></div>)}
          </div>
          {employeeNames.map((employeeName) => (
            <div key={employeeName} className="grid grid-cols-[180px_repeat(7,minmax(125px,1fr))] border-b">
              <div className="flex items-center gap-2 border-r p-3"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary-light text-xs font-bold text-primary">{employeeName.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</div><span className="truncate text-sm font-medium">{employeeName}</span></div>
              {days.map((day) => {
                const date = isoDate(day);
                const dayShifts = weekShifts.filter((shift) => shift.employeeName === employeeName && shift.date === date);
                return <div key={date} className="min-h-28 space-y-2 border-r p-2">{dayShifts.map((shift) => <button key={shift.id} type="button" onClick={() => openEdit(shift)} className={`w-full rounded-lg border p-2 text-left text-xs ${shiftTone(shift.shiftType)}`}><div className="flex items-start justify-between gap-1"><span className="font-semibold">{shift.startTime}–{shift.endTime}</span><Badge variant="outline" className="h-5 px-1 text-[10px]">{shift.shiftType}</Badge></div><p className="mt-1 truncate">{shift.project || 'Ei projektia'}</p></button>)}<button type="button" onClick={() => openCreate(date, employeeName)} className="flex w-full items-center justify-center rounded-md border border-dashed border-slate-200 py-1 text-slate-400 hover:border-primary hover:text-primary"><Plus size={13} /></button></div>;
              })}
            </div>
          ))}
          {!loading && employeeNames.length === 0 && <div className="p-12 text-center"><Users size={44} className="mx-auto mb-3 text-text-muted" /><p className="font-semibold">Ei henkilöstöä tai työvuoroja</p><p className="mt-1 text-sm text-text-secondary">Lisää henkilöstörekisteriin työntekijöitä ja luo ensimmäinen vuoro.</p></div>}
        </CardContent>
      </Card>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-xl">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 sm:col-span-2"><Label>Työntekijä *</Label>{employees.length > 0 ? <Select value={form.employeeId} onValueChange={selectEmployee}><SelectTrigger><SelectValue placeholder="Valitse työntekijä" /></SelectTrigger><SelectContent>{employees.filter((employee) => employee.status !== 'Eroonnut').map((employee) => <SelectItem key={employee.id} value={employee.id}>{employee.name}</SelectItem>)}</SelectContent></Select> : <Input value={form.employeeName} onChange={(event) => setForm((previous) => ({ ...previous, employeeName: event.target.value }))} />}</div>
          ...
          <DialogFooter className="sm:justify-between"><div>{editing && <Button variant="ghost" className="text-danger" onClick={() => { setDialogOpen(false); setDeleteTarget(editing); }}><Trash2 size={14} className="mr-1" /> Poista</Button>}</div><div className="flex gap-2"><Button variant="outline" onClick={() => setDialogOpen(false)} disabled={saving}>Peruuta</Button><Button onClick={() => void save()} disabled={saving}>{saving ? 'Tallennetaan…' : 'Tallenna'}</Button></div></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={Boolean(deleteTarget)} onOpenChange={(open) => !open && setDeleteTarget(null)}><DialogContent><DialogHeader><DialogTitle>Poista työvuoro</DialogTitle></DialogHeader><p className="text-sm text-text-secondary">Poistetaanko {deleteTarget?.employeeName} työvuoro päivältä {deleteTarget?.date}?</p><DialogFooter><Button variant="outline" onClick={() => setDeleteTarget(null)}>Peruuta</Button><Button variant="destructive" onClick={() => void removeSelectedShift()} disabled={saving}>Poista</Button></DialogFooter></DialogContent></Dialog>
    </motion.div>
  );
}
