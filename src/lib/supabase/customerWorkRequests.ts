...
const CUSTOMER_REQUEST_STATUSES: CustomerRequestStatus[] = [
...
function toRequest(row: Record<string, unknown>): CustomerWorkRequest {
  ...
  return {
    ...
    status: CUSTOMER_REQUEST_STATUSES.includes(status as CustomerRequestStatus)
...
export async function loadCustomerPortalProjects(organizationId: string): Promise<CustomerPortalProject[]> {
  const { data, error } = await supabase.rpc('customer_portal_projects', {
...
export async function createCustomerWorkRequest(values: {
  ...
  const { data, error } = await supabase.rpc('create_customer_work_request', {
    ...
    p_location_details: values.locationDetails || null,
    p_contact_name: values.contactName || null,
    p_contact_phone: values.contactPhone || null,
    p_requested_date: values.requestedDate || null,
    p_preferred_time: values.preferredTime || null,
    p_urgency: values.urgency,
    p_access_instructions: values.accessInstructions || null,
    p_safety_notes: values.safetyNotes || null,
  });
