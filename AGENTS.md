...
- **No self-registration.** The login page only signs in; accounts are created by
  an org admin. To exercise authenticated flows (the real app behind login), you
  need a pre-provisioned test account via `E2E_USER_EMAIL` / `E2E_USER_PASSWORD`.
  Without them you can still prove end-to-end health by submitting the login form
  and confirming the live backend returns an auth response.
